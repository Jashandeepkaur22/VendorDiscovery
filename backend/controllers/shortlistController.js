...
// Initialize the SDK inside the handler using the env key.
...
`;
      ...
      if (isVectorDb) {
        return [
          ...
          },
          {
            ...
            quotedSnippets: ["Forever free 1GB cluster for rapid prototyping."],
          },
        ];
      }
    ...
    if (!hasValidKey) {
      console.log("Using Mock Data (No Valid Gemini Key Provided)");
      await new Promise((resolve) => setTimeout(resolve, 1500));
      return res.status(200).json({ shortlist: getMockData() });
    }
    ...
    try {
      ...
      console.error(
        "Gemini API Error (likely Vercel restriction):",
        apiError.message,
      );
